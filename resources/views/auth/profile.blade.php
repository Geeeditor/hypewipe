...
@section('content')
    <section x-data="{ profile: false, terminateProfile: false, resetPassword: false }" class="flex justify-center items-center">
        <div class="relative w-[100%]">
                ...
                <div
                    ...
                    <div class="flex justify-between items-center bg-opacity-90 bg-cover bg-center my-2 px-4 py-3 rounded w-full text-white capitalize"
                        ...
                        <div x-data="{ tooltip: false }" class="relative my-2 w-2/3">
                            ...
                            <p
                                class="font-bold {{ $user->questJob && ($user->questJob->earnings ?? 0) > 0 ? 'text-green-400' : 'text-red-400' }} poppins-medium">
                    ...
                    <div class="flex justify-between items-center bg-opacity-90 bg-cover bg-center my-2 px-4 py-3 rounded w-full text-white capitalize"
                        ...
                        <div x-data="{ tooltip: false }" class="relative my-2 w-2/3">
                            ...
                            <div x-transition x-show="tooltip" @click.outside="tooltip = false"
                                class="hover:block top-6 left-0 z-10 absolute bg-white shadow-lg p-2 rounded-md w-[200px] text-black">
                                You must have a balance exceeding the required cost for the task; this field displays that
                                balance.</p>
                                <p class="my-1 text-xs poppins-light">- If you ever run low on credit remember to top up
                                    your wallet.</p>
                            </div>
                            <p
                                class="font-bold {{ $user->userWallet && $user->userWallet->wallet_balance > 0 ? 'text-green-400' : 'text-red-400' }} poppins-medium">
            ...
            <div class="flex justify-between items-center bg-gray-100 mx-3 mt-5 p-4 border border-gray-600 rounded-md">
                ...
                <button @click="profile = !profile" id="update-profile"
                    class="bg-blue-600 hover:bg-blue-800 px-3 py-2 rounded-sm text-white">Edit</button>
            </div>
            ...
            <div class="mx-3 mt-5 border border-gray-600 rounded-md">
                <div class="flex justify-between items-center gap-2 p-4">
                    ...
                    <img class="rounded-full h-[60px]" src="{{ $user->display_pic ? asset('') : $profilePic }}"
                        alt="">
                </div>
                <div class="flex justify-between items-center gap-2 p-4">
            ...
            <div class="flex justify-between items-center bg-gray-100 mx-3 mt-5 p-4 border border-gray-600 rounded-md">
                ...
                <div @click="terminateProfile = !terminateProfile"
                    class="bg-red-600 hover:bg-red-800 px-3 py-2 rounded-sm text-white cursor-pointer">Delete</div>

            </div>

            {{-- Update Profile picture modal --}}
            <div x-transition x-show="profile" @click.outside="profile = !profile" id="profile"
                class="z-50 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 mx-auto max-w-[425px]">
                <form method="POST" action="{{ route('profile.update') }}"
                    class="bg-white shadow-lg p-6 rounded-lg w-[80%] h-[70vh] overflow-y-auto">
                    @method('PUT')
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}"
                            class="mb-4 py-1 border-gray-600 border-b outline-none w-full">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="gender" class="mb-4 py-1 border-gray-600 border-b outline-none w-full">
                            <option
                                class="bg-white hover:bg-blue-100 text-gray-900 hover:text-blue-700 transition-colors duration-150"
        ...
        <div x-transition x-show="resetPassword" @click.outside="resetPassword = false" id="reset-password" class="z-50 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 mx-auto max-w-[425px]">
            ...
            {{-- Delete your profile --}}
            <div x-transition x-show="terminateProfile" @click.outside="terminateProfile = !terminateProfile"
                id="delete-profile"
                class="z-50 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 mx-auto max-w-[425px]">
                <form method="POST" action="{{ route('profile.destroy') }}" method="POST"
                    class="bg-white shadow-lg p-6 rounded-lg w-[80%] h-fit">
                    @method('delete')
                    @csrf
                    <h2 class="flex justify-between gap-2 mb-4 w-full text-base poppins-bold">
                        <span class="w-[70%] capitalize">Terminate your profile</span>
                        <span @click="terminateProfile = !terminateProfile"
                            class="text-red-700 cursor-pointer mdi mdi-close"></span>
                    </h2>
                    <p class="mb-4 text-gray-700 text-sm poppins-light">Are you sure you want to terminate your account?
                    ...
                    <button @click="showModal = false"
                        class="bg-red-600 hover:bg-red-800 mt-4 px-4 py-2 rounded-md w-full text-white">Delete
                        Account</button>
                    <div @click="terminateProfile = !terminateProfile"
                        class="bg-blue-600 hover:bg-blue-800 mt-4 px-4 py-2 rounded-md w-full text-white text-center cursor-pointer">
                        Cancel</div>
                </form>
